...
* Dashboard routes require a `dashboard_session` cookie issued by `/api/v1/auth/dev-login` (credentials sourced from environment variables).
* CORS configured from `ALLOWED_ORIGINS`.
* Cookies are HTTP-only and `SameSite=Lax` to satisfy Spotify OAuth redirects on `127.0.0.1`.

## Frontend Touchpoints

The React frontend consumes the authenticated dashboard APIs (accounts, ingest, playlists, manager, metrics) and surfaces the public playlist directory. End users interact with the public listing or submit a single album source without needing dashboard access.

This architecture enables high-volume ingest (10k+ albums), daily playlist rotations, and automated scaling across multiple Spotify accounts while centering operations on the Vibe Control Center dashboard.
